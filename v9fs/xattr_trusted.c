...
#if RHEL6_COMPAT
static int v9fs_xattr_trusted_get_inode(struct inode *inode, const char *name,
			             void *buffer, size_t size)
{
	...
	return v9fs_xattr_trusted_get(dentry, name, buffer, size, 0);
}

static int v9fs_xattr_trusted_set_inode(struct inode *inode, const char *name,
			const void *value, size_t size, int flags)
{
	...
	return v9fs_xattr_trusted_set (dentry, name, value, size, flags, 0);
}
#endif
...
#if RHEL6_COMPAT
	.get	= v9fs_xattr_trusted_get_inode,
	.set	= v9fs_xattr_trusted_set_inode,
#else
